...
export type RunOptions={memory?:{store:MemoryStore;owner:string};workspace:Workspace;extensions:Extensions;signal:AbortSignal;emit:(event:AgentEvent)=>void;turn?:typeof modelTurn;runId?:string;workspaceEvent?:(event:WorkspacePayload)=>void;connectExternal?:(signal:AbortSignal)=>Promise<Connection[]>};
...
export async function runAgent(input:RunInput,options:RunOptions):Promise<Completion>{
  ...
  const researchTool=(name:string)=>name==='core__read_webpage'||!name.startsWith('core__')&&!name.startsWith('memory__')&&/(search|fetch|browse|crawl|web)/i.test(name);
