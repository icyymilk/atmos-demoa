...
export function AgentTrace({events,live=false,ownerId}:{events:AgentEvent[];live?:boolean;ownerId?:string}){
  ...
  return <div className="agent-trace">
    ...
    {events.filter(e=>e.type!=='tool_end'&&e.type!=='iteration'&&e.type!=='budget').map((event,i)=>{
      if(event.type==='approval'&&event.approval)return <ApprovalCard key={event.approval.id} approval={event.approval} live={live} ownerId={ownerId}/>;
    ...
    {live&&!events.some(e=>e.type==='approval'&&e.approval?.status==='pending')&&last?.type!=='tool_start'&&iteration&&<div className="trace-thinking"><Loader2 size={13} className="spin"/>{iteration.text}</div>}
  </div>;
}
