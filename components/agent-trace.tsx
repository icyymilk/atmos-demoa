...
const labels:Record<string,string>={list_files:'列出文件',read_file:'读取文件',write_file:'写入文件',edit_file:'精确编辑',search_files:'检索项目',delete_file:'删除文件',read_webpage:'读取网页',update_tasks:'更新任务清单',read_memory:'读取项目记忆',write_memory:'保存项目记忆',load_skill:'加载 Skill',validate_app:'验证应用',complete_task:'交付应用',web_search_exa:'搜索互联网',web_fetch_exa:'读取网络资料',get_code_context_exa:'检索技术资料',current_time:'查询时间',calculate:'计算'};
export function AgentTrace({events,live=false}:{events:AgentEvent[];live?:boolean}){
  const starts=events.filter(e=>e.type==='tool_start'),last=events.at(-1);
  return <div className="agent-trace">
    <div className="trace-heading"><Terminal size={14}/><strong>{live?'Agent 正在执行':'Agent 执行记录'}</strong><span>{starts.length} 次调用</span></div>
    {events.filter(e=>e.type!=='tool_end'&&e.type!=='iteration').map((event,i)=>{
      ...
      return <div className={`trace-note trace-${event.type}`} key={i}>{event.type==='assistant'?<Bot size={13}/>:event.type==='complete'?<Check size={13}/>:<Clock3 size={13}/>}<p>{event.text}</p></div>;
    })}
    {live&&last?.type==='iteration'&&<div className="trace-thinking"><Loader2 size={13} className="spin"/>{last.text}</div>}
  </div>;
}
