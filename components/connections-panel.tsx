...
export function ConnectionsPanel() {
  ...
  async function action(provider: Provider, action: 'connect' | 'test' | 'disconnect') {
    ...
    try {
      const response = await fetch('/api/connections', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ provider, action, ...(action === 'connect' ? { token } : {}) }) });
      const data = await response.json() as { connections: ConnectionStatus[]; error?: string };
  ...
  return <section className="connections-section" aria-label="外部服务连接">
    ...
    <div className="connections-account"><ShieldCheck size={16}/><p><strong>当前是访客会话</strong> · 尚未开放账户注册、登录和跨设备同步。连接第三方服务不会创建 Atmos 账户。</p></div>
